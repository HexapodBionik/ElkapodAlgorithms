...
point = ax.scatter(coordinate_points_x, coordinate_points_y, coordinate_points_z, color='red', s=100, label="Interpolated points")
...
def animate(i):
    theta1, theta2, theta3 = [float(polys[x](np.array([i * 0.005]))) for x in range(3)]
    forward_coordinates = kinematics_solver.forward(theta1, theta2, theta3)

    # Update the 3D scatter plot directly
    point._offsets3d = ([forward_coordinates[0]], [forward_coordinates[1]],
                        [forward_coordinates[2]])

    return point,


ani = FuncAnimation(fig, animate, interval=len(x))
